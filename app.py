...
# ---------------------------------------------------------
# 10. قسم أسهم المستثمرين (محدث بالكامل مع التحليلات والرسومات البيانية)
# ---------------------------------------------------------
    ...
    # جلب بيانات المستثمرين والعقارات
    conn = get_connection()
    query = """
        SELECT 
            i.id, 
    ...
    # -----------------------------------------------------
    # التاب الأول: لوحة التحليلات والرسومات البيانية
    # -----------------------------------------------------
    with tab_inv1:
        if not df_raw_inv.empty:
            # 1. بطاقات المؤشرات المالية الرئيسية (KPI Metrics)
            ...
            # 2. الرسم البياني الأول: توزيع المحفظة الاستثمارية والرسم التراكمي
            col_chart1, col_chart2 = st.columns(2)
            ...
            # 3. رسم بياني إضافي: تحليل معدل العائد الاستثماري (ROI)
    ...
    # -----------------------------------------------------
    # التاب الثاني: جدول توزيع الحصص الاحترافي
    # -----------------------------------------------------
    with tab_inv2:
        ...
        if not df_raw_inv.empty:
            ...
            with col_del1:
                inv_to_del = st.selectbox("اختر رقم معرف الحساب (ID) لإلغاء الشراكة أو حذفه", df_display['ID'].tolist())
            with col_del2:
                st.write("")
                st.write("")
                if st.button("🗑️ حذف سجل المستثمر المحدد", use_container_width=True):
                    conn = get_connection()
                    cursor = conn.cursor()
    ...
    # -----------------------------------------------------
    # التاب الثالث: إضافة وربط مستثمر جديد
    # -----------------------------------------------------
    with tab_inv3:
...
# ---------------------------------------------------------
    ...
    if os.path.exists("uploads"):
        ...
        st.info("مجلد المستندات فارغ.")# 2. إدارة قاعدة البيانات
# ---------------------------------------------------------
DB_FILE = "mh_group_erp.db"
