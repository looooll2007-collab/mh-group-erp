...
# ---------------------------------------------------------
# 11. رفع وإدارة المستندات والعقود (مُحدّث بكامل المميزات)
# ---------------------------------------------------------
elif menu == "رفع المستندات":
    st.header("📁 مركز أرشفة وإدارة المستندات والعقود")
    
    # 1. نموذج رفع المستندات والتصنيف
...
# ---------------------------------------------------------
# 12. إغلاق الاتصال بقاعدة البيانات في النهاية
# ---------------------------------------------------------
if conn:
    conn.close()        background-color: #161b22 !important;
        border-right: 1px solid #30363d;
    }
    section[data-testid="stSidebar"] .stRadio label {
        font-size: 1.15rem !important;
...
# ---------------------------------------------------------
    ...
    with tab3:
        ...
        with st.form("fin_form"):
            ...
            submit = st.form_submit_button("تسجيل المعاملة")
            if submit:
                # تصحيح الخطأ: وضع 4 علامات استفهام لمطابقة الـ 4 أعمدة
                cursor.execute("INSERT INTO finance (type, amount, category, description) VALUES (?, ?, ?, ?)",
                               (fin_type, amount, category, description))
                conn.commit()
                st.success("تم تسجيل المعاملة المالية!")
